...
const MyOrders = () => {
    ...
    return (
        <div className='min-h-screen bg-[#faf7f2] mb-20'>
           ...
           {tabs.map((tab)=>(
            <button onClick={()=>setActiveTab(tab)} className={`px-4 py-2 text-sm font-medium rounded-xl whitespace-nowrap transition-colors ${activeTab===tab?"bg-[#1b3022] text-white":"bg-white text-light hover:bg-yellow-100"}`}>{tab==='all'?"All Orders":tab}</button>
           ))}
            </div>
            {/* Orders-List */}
            {loading ? (
...
{orders.map((order)=>(
    <Link key={order._id} to={`/orders/${order._id}`} className='block max-w-4xl bg-white rounded-2xl p-5 hover:shadow transition-all'>
      ...
      {/* left */}
      <div>
        <p className='text-sm font-medium text-green-500'>Order #{order._id.slice(-8).toUpperCase()}</p>
        <div className='flex items-center gap-2 mt-1'>
           <Calendar className='size-3 text-light'/>
           <span className='text-xs text-light'>{new Date(order.createdAt).toLocaleDateString("en-US", {
  month: "short",
  day: "numeric",
  year: "numeric"
})}</span>
...
{order.items.slice(0,4).map((item,i)=>(
    <img key={i} alt={item.name} src={item.image} className='size-12 sm:size-16 rounded-lg object-cover border border-black/20'/>
))}
{order.items.length>4 && <div className='size-12 sm:size-16 rounded-lg bg-amber-200 flex-center text-xs font-semibold text-light'></div>}
</div>
        {/* total items-price */}
