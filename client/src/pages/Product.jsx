...
const Product = () => {
  ...
  const { data: products = [], isLoading, isFetching } = useGetAllProductQuery({
    category,
    minPrice,
    maxPrice,
    sort
  ...
  return (
    <div className="min-h-screen bg-[#FAF7F2]">
        ...
        <div className="flex gap-8 xl:gap-10 shrink-0">
          ...
          <main className="flex-1">
            ...
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h1 className="text-2xl font-semibold text-tight">
                      {activeCategory ? activeCategory.name : "All Products"}
                    </h1>
                ...
                <div className="flex flex-col lg:items-center gap-3">
                    ...
                    <button 
                      className="lg:hidden flex items-center gap-2 px-3 py-2 text-sm bg-[#FFFFFF] rounded-xl border border-[#E5E7EB] hover:bg-amber-50 transition-colors" 
                      onClick={() => setMobileFiterOpen(true)}
                    >
            ...
            {isLoading || isFetching ? (
              ...
              <div className="text-center py-16">
                ...
                <p className="text-sm text-light mb-4">Try adjusting your filters or search terms</p>
                <button onClick={clearFilters} className="px-5 py-2 text-sm font-medium bg-[#2D4A35] text-white rounded-xl hover:bg-[#122016] transition-colors">
                  Clear Filters
                </button>
              </div>
            ) : (
              ...
              <div className="w-full flex justify-center py-4">
                {Array.from({ length: totalpages }).map((_, i) => (
                  <button 
                    ...
                    className={`size-9 rounded-lg text-sm font-medium transition-colors ${page === i + 1 ? "bg-green-500 text-white " : "bg-white text-light hover:bg-yellow-500"}`}
                  >
                    {i + 1}
                  </button>
                ))}
      ...
      {mobileFiterOpen && (
        <>
          <div className="fixed inset-0 z-50 bg-black/20" onClick={() => setMobileFiterOpen(false)} />
          <div className="fixed bottom-0 left-0 right-0 bg-white z-50 rounded-t-2xl max-h-[80vh] overflow-y-auto">
