...
const OrderDetails = () => {
  ...
  useEffect(() => {
    if (rawData) {
      ...
      const addressObj =
        currentOrder?.shippingAddress?.[0] || currentOrder?.shippingAddress;
      if (addressObj) {
        setLiveLocation({
          lat: addressObj.lat || "",
          lng: addressObj.lng || "",
 ...
 console.log(order)
  const shipping = Array.isArray(order.shippingAddress)
              ...
              order.status !== "Cancelled" && (
                <div className="bg-white rounded-2xl p-5 flex items-center justify-between shadow-sm">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-[#1b3022]">
                        {deliveryPartner.name}
                      </p>
                      <p className="text-xs text-gray-500 capitalize">
                        {deliveryPartner.vehicleType} • Delivery Partner
                      </p>
                    </div>
                  </div>
                  <a
                    href={`tel:${deliveryPartner.phone}`}
                    className="p-2.5 bg-[#1B3022] rounded-xl hover:bg-yellow-600 transition-colors"
                  >
              ...
              <div className="mt-4 pt-3 border-t border-gray-200 space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span>
                    {order?.deliveryFee === 0
                      ? "Free"
                      : `${currency}${order?.deliveryFee?.toFixed(2) || "0.00"}`}
                  </span>
                </div>
