  ...
  return (
    <section className="mt-10 ">
      ...
      <div className="bg-white/50 rounded-2xl p-6 md:p-8">
        {/* Summary row */}
        <div className="flex flex-col md:flex-row gap-8 mb-8 pb-8 border-b border-[#e5e7eb]">
          {/* Average */}
          <div className="flex-center flex-col md:min-w-[160px] lg:w-1/3">
            ...
            <div className="flex items-center gap-0.5 mt-2 mb-1">
              {[1, 2, 3, 4, 5].map((s) => (
                <StarIcon key={s} className={`size-4 ${s <= Math.round(product.rating) ? "text-[#f59e0b] fill-[#f59e0b]" : "text-app-border"}`} />
              ))}
            </div>
            ...
            {breakdown.map((count, i) => (
              <div key={i} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-2.5 bg-[#e5e7eb] rounded-full overflow-hidden">
                  <div className="h-full bg-[#f59e0b] rounded-full transition-all duration-500" style={{ width: `${(count / maxCount) * 100}%` }} />
                </div>
        ...
        <div className="space-y-6">
          {reviews.map((review) => (
            <div key={review.id} className="flex gap-4">
              <div className="size-10 rounded-full bg-[#e6e6e3] text-app-green justify-center shrink-0 text-sm font-semibold flex items-center">
                {review.avatar}
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-0.5 mb-2">
                  {[1, 2, 3, 4, 5].map((s) => (
                    <StarIcon key={s} className={`size-3.5 ${s <= review.rating ? "text-[#f59e0b] fill-[#f59e0b]" : "text-[#9e9795]"}`} />
                  ))}
                </div>
